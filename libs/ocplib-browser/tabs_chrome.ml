...
class type tabs = object
  ...
  method create : tab t -> (tab t -> unit) callback optdef -> unit meth
  ...
  method getAllInWindow : int optdef -> (tab t js_array t -> unit) callback -> unit meth
  method getCurrent : (tab t -> unit) callback -> unit meth
  method getSelected : int optdef -> (tab t -> unit) callback -> unit meth
  ...
  method sendRequest : int -> 'a t -> (json t -> unit) callback optdef -> unit meth
  ...
  method onActiveChanged : (int, 'a t) event2 t prop
...
let getAllInWindow ?id f = tabs##getAllInWindow(Optdef.option id, wrap_callback f)
let getCurrent f = tabs##getCurrent(wrap_callback f)
let getSelected ?id f = tabs##getSelected(Optdef.option id, wrap_callback f)
...
let sendRequest ?callback id request =
  tabs##sendRequest(id, request, optdef_wrap callback)
...
let onActiveChanged handler =
  tabs##onActiveChanged##addListener(wrap_callback handler)
...
let onHighlightChanged handler =
  tabs##onHighlightChanged##addListener(wrap_callback handler)
...
let onSelectionChanged handler =
  tabs##onSelectionChanged##addListener(wrap_callback handler)
